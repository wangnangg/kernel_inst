...
EXPORT_SYMBOL(aging_trigger_tcp_congestion);
/*my code end */
...
static void bdi_debug_init(void)
{
	...
	/*my code begin*/
	junk = debugfs_create_u32("aging_trigger_mlock", 0666, 0, &aging_trigger_mlock);
	if (!junk) 
	{
		// Abort module load.
	...
	{
		// Abort module load.
		printk(KERN_ALERT "aging_trigger_sched: failed to create /sys/kernel/debug/aging_trigger_sched\n");
	}
	
	junk = debugfs_create_u32("aging_trigger_inotify", 0666, 0, &aging_trigger_inotify);
	if (!junk) 
	{
		// Abort module load.
		printk(KERN_ALERT "aging_trigger_inotify: failed to create /sys/kernel/debug/aging_trigger_inotify\n");
	}

	junk = debugfs_create_u32("aging_trigger_cifs", 0666, 0, &aging_trigger_cifs);
	if (!junk) 
	{
		// Abort module load.
	...
	junk = debugfs_create_u32("aging_trigger_tcp_congestion", 0666, 0, &aging_trigger_tcp_congestion);
	if (!junk) 
	{
		// Abort module load.
		printk(KERN_ALERT "aging_trigger_tcp_congestion: failed to create /sys/kernel/debug/aging_trigger_tcp_congestion\n");
	}
	/*my code end */
